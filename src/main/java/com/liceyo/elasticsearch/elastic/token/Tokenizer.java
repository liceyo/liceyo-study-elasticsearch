package com.liceyo.elasticsearch.elastic.token;
...
public class Tokenizer {
     ...
     * @return 分词结果
     */
    public static AnalyzeResponse analyze(String statement) {
        return analyze(statement,DEFAULT_TOKENIZER);
    }
     ...
     * @return 分词结果
     */
    public static AnalyzeResponse analyze(String statement, String analyzer){
        AnalyzeRequestBuilder requestBuilder = new AnalyzeRequestBuilder(transportClient, AnalyzeAction.INSTANCE, ConstantValue.SEARCH_INDEX_NAME, statement);
        requestBuilder.setAnalyzer(analyzer);
        return requestBuilder.execute().actionGet();
    }
     ...
     * @return 解析结果
     */
    public static List<String> analyzeAndAnalysis(String statement,String analyzer){
        AnalyzeResponse response=analyze(statement,analyzer);
        return response.getTokens().stream().map(AnalyzeResponse.AnalyzeToken::getTerm).collect(Collectors.toList());
    }
     ...
     * @return 解析结果
     */
    public static List<String> analyzeAndAnalysis(String statement){
        AnalyzeResponse response=analyze(statement);
        return response.getTokens().stream().map(AnalyzeResponse.AnalyzeToken::getTerm).collect(Collectors.toList());
    }
